...
void* mm_malloc(size_t size) {
  ...
  if (ptr_free_block == NULL) {
    ...
    // expand the heap: request_more_space
  ...
  if (block_size - MIN_BLOCK_SIZE >= req_size) {
    // Block is too big and we need to split

    // set the size of ptr_free_block = req_size and no more
    ptr_free_block->size_and_tags = SIZE(req_size)
     | (ptr_free_block->size_and_tags & TAG_PRECEDING_USED);

    // Use size to get the pointer to the new free block
    block_info* new_free_block = UNSCALED_POINTER_ADD(ptr_free_block, req_size);

    // create a new free block after ptr_free_block
    new_free_block->size_and_tags = block_size - req_size;
    new_free_block->size_and_tags += TAG_PRECEDING_USED;

    // add footer to new_free_block, add the size of the new block - word size to get address of footer
    block_info* new_free_block_footer = UNSCALED_POINTER_ADD(new_free_block, block_size - req_size - WORD_SIZE);
    new_free_block_footer->size_and_tags = new_free_block->size_and_tags;

    // add new free block to free list
    insert_free_block(new_free_block);
  } else {
    // set the next block's preceeding used tags
    block_info* next_block = UNSCALED_POINTER_ADD(ptr_free_block, block_size);
    next_block->size_and_tags = next_block->size_and_tags | TAG_PRECEDING_USED;
    if (next_block->size_and_tags & TAG_USED == 0) {
      // The next block is free, we also need to set its footer
      size_t next_block_size = SIZE(next_block->size_and_tags);
      block_info* next_block_footer = UNSCALED_POINTER_ADD(next_block, next_block_size - WORD_SIZE);
      next_block_footer->size_and_tags = next_block->size_and_tags;
    }
  }
...
*/
  ...
  following_block = UNSCALED_POINTER_ADD(block_to_free, block_size);

  // Convert the given used block into a free block by changing its used tag

  block_to_free->size_and_tags = block_to_free->size_and_tags & ~TAG_USED;

  // Update size_and_tags appropriately, change preceding used tag from the next free block

  following_block->size_and_tags = following_block->size_and_tags & ~TAG_PRECEDING_USED;

  // update the footer size_and_tags

  block_info* new_free_block_footer = UNSCALED_POINTER_ADD(block_to_free, block_size - WORD_SIZE);
  new_free_block_footer->size_and_tags = block_to_free->size_and_tags;

  // Reinsert free block into the head of the free list
